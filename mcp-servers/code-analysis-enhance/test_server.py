...
def test_file_dedup_state_machine():
    ...
    r1 = _text(server._tool_file_dedup({"path": target}))
    r2 = _text(server._tool_file_dedup({"path": target}))
...
def test_file_dedup_missing():
    r = _text(server._tool_file_dedup({"path": r"C:\no_such_file_xyz.txt"}))
    assert r["ok"] is False


def test_change_impact_extracts_symbols():
    repo = str(SERVER.parent.parent)  # global-workspace
    rel = "code-analysis-enhance/server.py"
    r = _text(server._tool_change_impact({"repo_path": repo, "changed_files": [rel]}))
    assert r["ok"] is True
    res = r["results"][0]
...
def test_change_impact_missing_repo():
    r = _text(server._tool_change_impact({"repo_path": r"C:\no_repo", "changed_files": []}))
    assert r["ok"] is False


def test_lesson_recall_hits_warnings():
    r = _text(server._tool_lesson_recall({"task_description": "修复 Python 依赖缺失 ModuleNotFoundError"}))
    assert r["ok"] is True
...
def test_aether_probe():
    r = _text(server._tool_aether_probe({}))
    assert r["ok"] is True
...
def test_code_context_python_ast():
    ...
    r = _text(server._tool_code_context({"path": target, "cursor_line": 1}))
    assert r["ok"] is True
    meta = r["metadata"]
...
def test_aether_agent_parse_edits():
    ...
    r = _text(server._tool_aether_agent_parse({"response": resp}))
    assert r["ok"] is True
...
def test_aether_lang_support():
    r = _text(server._tool_aether_lang_support({"path": r"C:\x\main.py"}))
    assert r["detected_language"] == "python"
    assert r["supported"] is True
    r2 = _text(server._tool_aether_lang_support({"path": r"C:\x\main.gd"}))
    assert r2["detected_language"] == "gdscript"


def test_aether_goto_parse():
    r = _text(server._tool_aether_goto_parse({"goto": "src/main.rs:42:7"}))
    assert r["path"] == "src/main.rs"
    assert r["line"] == 42
    assert r["column"] == 7
    assert r["zero_based_line"] == 41
    r2 = _text(server._tool_aether_goto_parse({"goto": "onlypath"}))
...
def test_lsp_position_convert_byte_to_position():
    ...
    text = "abc\ndef\nghi"
    r = _text(server._tool_lsp_position_convert({"text": text, "direction": "byte_to_position", "byte_offset": 4}))
    assert r["position"]["line"] == 1
    assert r["position"]["character"] == 0
    r2 = _text(server._tool_lsp_position_convert({"text": text, "direction": "byte_to_position", "byte_offset": 0}))
...
def test_lsp_position_convert_position_to_byte():
    ...
    text = "你好\nworld"
    r = _text(server._tool_lsp_position_convert({"text": text, "direction": "position_to_byte", "line": 0, "character": 2}))
    # "你好" 每个字 3 字节 UTF-8；character=2 是 UTF-16 码元数（每字 1）
    assert r["byte_offset"] == 6
    r2 = _text(server._tool_lsp_position_convert({"text": text, "direction": "position_to_byte", "line": 1, "character": 2}))
...
def test_lsp_semantic_tokens_decode():
    ...
    r = _text(server._tool_lsp_semantic_tokens_decode({"data": data}))
...
def test_symbol_patterns_sanitized():
    ...
    syms = server._extract_symbols(content, ".py", extra)
...
def test_symbol_patterns_redos_rejected():
    ...
    for p in redos_pats:
        assert server._is_redos_risky(p), f"应拒绝 ReDoS 结构: {p}"
    safe_pats = [r"^\s*def\s+(\w+)", r"\w+", r"class\s+([A-Z]\w*)"]
    for p in safe_pats:
        assert not server._is_redos_risky(p), f"不应误杀正常正则: {p}"
...
def test_symbol_patterns_redos_bypass_rejected():
    ...
    for p in bypass_pats:
        assert server._is_redos_risky(p), f"应拒绝绕过形态: {p}"
...
def test_symbol_patterns_no_capture_group():
    ...
    content = "def foo():\n    pass\nbar = 1\n"
    # \w+ 无捕获组，匹配时用整个匹配
    syms = server._extract_symbols(content, ".py", [r"\w+"])
...
def test_symbol_patterns_sandwich_rejected():
    ...
    for p in sandwich:
        assert server._is_redos_risky(p), f"应拒绝三明治形态: {p}"
...
def test_symbol_patterns_bounded_quantifier_chain():
    ...
    for p in chains:
        assert server._is_redos_risky(p), f"应拒绝有界量词链: {p}"
...
def test_symbol_patterns_unbounded_quantifier_chain():
    ...
    for p in chains:
        assert server._is_redos_risky(p), f"应拒绝无上限量词链: {p}"
...
def test_symbol_patterns_two_quantifier_overlap():
    ...
    for p in chains:
        assert server._is_redos_risky(p), f"应拒绝两量词重叠链: {p}"
...
def test_symbol_patterns_open_interval_chain():
    ...
    for p in chains:
        assert server._is_redos_risky(p), f"应拒绝开区间量词链: {p}"
...
def test_change_impact_large_file_rejected(tmp_path):
    ...
    repo.mkdir()
    big = repo / "huge.py"
    big.write_bytes(b"x" * (server._SYMBOL_CONTENT_LIMIT * 3))
    r = _text(server._tool_change_impact({"repo_path": str(repo), "changed_files": ["huge.py"]}))
    assert r["ok"] is True
...
def test_lsp_position_convert_text_limit():
    ...
    r = _text(server._tool_lsp_position_convert({"text": "a" * (server._SYMBOL_CONTENT_LIMIT + 10),
...
def test_change_impact_path_traversal(tmp_path):
    ...
    r = _text(server._tool_change_impact({
        "repo_path": str(repo),
...
def test_semantic_tokens_oom_guard():
    ...
    r = _text(server._tool_lsp_semantic_tokens_decode({"data": "[" + ",".join(["1"] * 300000) + "]"}))
    assert r["ok"] is False
...
def test_semantic_tokens_bad_data():
    """非列表 data 被拒绝。"""
    r = _text(server._tool_lsp_semantic_tokens_decode({"data": {"not": "list"}}))
    assert r["ok"] is False
...
def test_lsp_edit_merge_adjacent():
    ...
    r = _text(server._tool_lsp_edit_merge({"edits": edits}))
...
def test_code_context_cross_file_references(tmp_path):
    ...
    r = _text(server._tool_code_context({
...
def test_lesson_recall_rx_memory():
    ...
    r = _text(server._tool_lesson_recall({"task_description": "缓存优化 正确性优先 不要命中率"}))
    assert r["ok"] is True
    assert isinstance(r["lessons"], list)
    assert isinstance(r["antipatterns"], list)
...
def test_lsp_request_ignores_notifications_until_timeout():
    ...
    client = server._LspClient.__new__(server._LspClient)
...
def test_lsp_retry_only_32801():
    ...
    assert server._is_retryable_error(err_32801) is True
    assert server._is_retryable_error(null_result) is False
...
def test_aether_model_provider_all():
    r = _text(server._tool_aether_model_provider({}))
    assert r["ok"] is True
...
def test_aether_model_provider_deepseek():
    r = _text(server._tool_aether_model_provider({"provider": "deepseek"}))
    assert r["ok"] is True
...
def test_aether_model_provider_unknown():
    r = _text(server._tool_aether_model_provider({"provider": "nope"}))
    assert r["ok"] is True
    assert r["providers"] == []


def test_lsp_query_unsupported_lang():
    r = _text(server._tool_lsp_query({"language_id": "brainfuck", "request": "hover",
...
def test_lsp_query_python_real():
    ...
    r = _text(server._tool_lsp_query({"language_id": "python", "request": "hover",
...
def test_lsp_query_rust_real():
    ...
    code = "fn main() {\n    println!(\"hi\");\n}\n"
    r = _text(server._tool_lsp_query({"language_id": "rust", "request": "hover",
...
def test_lsp_query_cpp_real():
    ...
    r = _text(server._tool_lsp_query({"language_id": "cpp", "request": "hover",
...
def test_lsp_query_typescript_real():
    ...
    r = _text(server._tool_lsp_query({"language_id": "typescript", "request": "hover",
...
def test_lsp_no_process_leak():
    ...
    for lid, path, text, line, ch in samples:
        r = _text(server._tool_lsp_query({"language_id": lid, "request": "hover",
