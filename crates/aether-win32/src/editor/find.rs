...
impl FindState {
    ...
    pub fn find_all(&mut self, content: &TabContent) {
        ...
        if self.query == self.last_query
            && self.result_version == content.buffer_version
        ...
        for line_idx in 0..total_lines {
            if let Some(line) = content.buffer.get_line(line_idx) {
                ...
                while let Some(pos) = line[start..].find(&query) {
                    ...
                    self.results.push((line_idx, abs_pos));
    ...
    pub fn find_next(&mut self, content: &mut TabContent) {
        ...
        if !self.results.is_empty() {
            self.active_index = (self.active_index + 1) % self.results.len();
            let (line, col) = self.results[self.active_index];
            // P2-6: 选区末尾对齐到字符边界；cursor_col 置于匹配末尾以符合编辑器约定
            let end_col = clamp_to_char_boundary(content, line, col + self.query.len());
    ...
    pub fn find_prev(&mut self, content: &mut TabContent) {
        ...
        if !self.results.is_empty() {
            if self.active_index == 0 {
                self.active_index = self.results.len() - 1;
            } else {
                self.active_index -= 1;
            }
            let (line, col) = self.results[self.active_index];
    ...
    pub fn replace_current(&mut self, content: &mut TabContent) -> bool {
        if self.results.is_empty() || self.active_index >= self.results.len() {
            return false;
        }
        let (line, col) = self.results[self.active_index];
        ...
        content.buffer.insert(pos, &self.replace_text);
    ...
    pub fn toggle_find(&mut self, content: &TabContent) {
        ...
        if !self.visible {
            ...
            self.focus = FindReplaceFocus::None;
        } else {
            self.focus = FindReplaceFocus::FindQuery;
        }
    ...
    pub fn toggle_replace(&mut self, content: &TabContent) {
        ...
        self.visible = self.replace_visible || self.visible;
        if !self.visible {
            self.focus = FindReplaceFocus::None;
        } else {
            self.focus = if self.replace_visible {
...
impl EditorState {
    ...
    pub fn replace_all(&mut self) -> usize {
        if self.find.query.is_empty() || self.find.query == self.find.replace_text {
            return 0;
        }
        self.find.find_all(&self.content);
        let count = self.find.results.len();
        if count == 0 {
            return 0;
        }
        ...
        let replace_text = self.find.replace_text.clone();
